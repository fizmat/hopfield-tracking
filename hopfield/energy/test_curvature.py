...
from hopfield.energy.curvature import curvature_pairwise, curvature_energy, curvature_energy_matrix, segment_adjacent_pairs
...
def test_curvature_pairwise():
    ...
    assert_array_equal(cosines, [1, 1/r2, 1/r2])
    assert_array_equal(rr, [1, r2, 2*r2])


def test_curvature_energy():
    assert_array_equal(curvature_energy(np.zeros((0)), np.zeros((0))), np.zeros(0))

    cosines = np.array([1, 1, 0.5, 0.5, -0.5])
    rr = np.array([1, 2, 1, 3, 5])
    assert_array_equal(curvature_energy(cosines, rr),
                       [1, 1/2, 1/8, 1/24, 0])
    assert_array_equal(curvature_energy(cosines, rr, cosine_power=1),
                       [1, 1/2, 1/2, 1/6, 0])
    assert_array_equal(curvature_energy(cosines, rr, cosine_threshold=0.8),
                       [1, 1/2, 0, 0, 0])
    assert_array_equal(curvature_energy(cosines, rr, distance_prod_power_in_denominator=0),
                       [1, 1, 1/8, 1/8, 0])
