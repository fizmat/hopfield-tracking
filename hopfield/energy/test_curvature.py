...
def test_curvature_energy_matrix():
    ...
    null_segment = np.empty((0, 2), dtype=int)
    w = curvature_energy_matrix(null_pos, null_segment, segment_adjacent_pairs(null_segment), 1., 1.)
    assert w.shape == (0, 0)

    pos = np.array([[0., 0], [1., 0], [2., 0], [2, 1], [2, -1], [3, 0], [3, 2]])
    w = curvature_energy_matrix(pos, null_segment, segment_adjacent_pairs(null_segment), 1., 1.)
    assert w.shape == (0, 0)
